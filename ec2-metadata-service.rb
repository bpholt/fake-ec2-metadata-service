...
get '/latest/meta-data/iam/security-credentials/:role' do
    if (File.file?(creds_file))
        ...
        if (inifile.has_section?('default'))
            aws_credentials = { code: "Success", last_updated: Time.now.utc.iso8601, type: "AWS-HMAC", access_key_id: inifile['default']['aws_access_key_id'], secret_access_key: inifile['default']['aws_secret_access_key'], token: "", expiration: (Time.now.utc + 31622400).iso8601 }
